...
void exchange(Solver *solver, double *grid) {
  ...
  MPI_Neighbor_alltoallw(grid, counts, solver->sdispls, solver->bufferTypes,
                         grid, counts, solver->rdispls,
                         solver->bufferTypes, solver->comm);
}
...
void shift(Solver *solver) {
  ...
  double *buf = g + 1;
  MPI_Irecv(buf, 1, solver->bufferTypes[BOTTOM], solver->neighbours[BOTTOM], 0,
  ...
  MPI_Isend(buf, 1, solver->bufferTypes[TOP], solver->neighbours[TOP], 0,
  ...
  MPI_Irecv(buf, 1, solver->bufferTypes[LEFT], solver->neighbours[LEFT], 1,
  ...
  MPI_Isend(buf, 1, solver->bufferTypes[RIGHT], solver->neighbours[RIGHT], 1,
            solver->comm, &requests[3]);
...
void initSolver(Solver *solver, Parameter *params) {
  ...
  int dims[NDIMS] = {0, 0}; // Total freedom to MPI
  ...
  MPI_Cart_coords(solver->comm, solver->rank, NDIMS, solver->coords);
  ...
  // No need to account for ghost cells in the stride since you're just grabbing
  // a continuous block.
  ...
  // Store if boundary
  solver->isBndry[BOTTOM] = isBoundary(solver, BOTTOM);
  solver->isBndry[TOP] = isBoundary(solver, TOP);
  solver->isBndry[LEFT] = isBoundary(solver, LEFT);
  solver->isBndry[RIGHT] = isBoundary(solver, RIGHT);
#ifdef VERBOSE
  printConfig(solver);
#endif
}
...
void solve(Solver *solver) {
  ...
  while ((res >= epssq) && (it < itermax)) {
    ...
    if (solver->isBndry[BOTTOM] || solver->isBndry[TOP]) {
      for (int i = 1; i < imaxLocal + 1; i++) {
        if (solver->isBndry[BOTTOM])
          P(i, 0) = P(i, 1);
        if (solver->isBndry[TOP])
          P(i, jmaxLocal + 1) = P(i, jmaxLocal);
      }
    }

    if (solver->isBndry[LEFT] || solver->isBndry[RIGHT]) {
      for (int j = 1; j < jmaxLocal + 1; j++) {
        if (solver->isBndry[LEFT])
          P(0, j) = P(1, j);
        if (solver->isBndry[RIGHT])
          P(imaxLocal + 1, j) = P(imaxLocal, j);
      }
    }

    /* TODO: add global MPI_SUM reduction for res */
    MPI_Allreduce(MPI_IN_PLACE, &res, 1, MPI_DOUBLE, MPI_SUM, solver->comm);
...
static double maxElement(Solver *solver, double *m) {
  ...
  MPI_Allreduce(MPI_IN_PLACE, &maxval, 1, MPI_DOUBLE, MPI_MAX, solver->comm);
...
void normalizePressure(Solver *solver) {
  ...
  MPI_Allreduce(MPI_IN_PLACE, &avgP, 1, MPI_DOUBLE, MPI_SUM, solver->comm);
...
void setSpecialBoundaryCondition(Solver *solver) {
  ...
  /* parabola. Use solver->dims to adjust the formula according to rank */
    ...
    if (solver->isBndry[LEFT]) {
      ...
      double y;
      int j_global_offset = 0;
      for (int jrank = 0; jrank < solver->coords[JDIM]; jrank++) {
        j_global_offset += sizeOfRank(jrank, solver->dims[JDIM], jmax);
      }
      for (int j = 1; j < jmaxLocal + 1; j++) {
        int j_global = j_global_offset + j;
        y = mDy * (j_global - 0.5);
